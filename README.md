...
- **打开路线自动取景（v6.5.3）**：进入路线或复制副本后，地图自动 `setView` 到适合范围：按视口 12%×15% 留白计算 zoom（cap 8），再把 bounds 中心像素右移约 7.5% 视口宽以补偿左侧 220px 半透明玻璃面板——路线在「扣除面板的可见区」内视觉居中，左缘不贴面板。编辑过程（加站 / 拖拽 / 换天）不打断用户视角；保存归位不重取景
