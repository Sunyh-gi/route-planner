...
- **🏠 主页画廊**：打开平台先看到一张全屏主页，列出全部路线卡片；每张卡显示路线名 + 「X 天 X 晚」行程时长，无任何「内置」徽章。顶部三件套：＋ 新建路线 / ⚙ 设置 / 🔄 刷新
...
- `_smoke.js`：三阶段冒烟（puppeteer-core + 系统 Edge，`file://` 直开场景）
  - **阶段 A**（18 项，无 Token 只读）：主页画廊两张卡无徽章 / 目录已注册 / 主页三件套（新建 / 设置 / 刷新）/ 点卡懒加载 cx 11 点进入视图模式（搜索 / 保存 / 编辑按钮 / 拖拽全隐藏、无 day-stepper）/ ⋯ 菜单三选项无图标 / 点「编辑路线」切编辑模式（66 个行内 ops + 11 个可拖拽行）/ 搜索预览 + 液态玻璃卡片 / 加站 12 点两天 dirty / 相邻天颜色按 routeId 种子洗牌显著不同 / 编辑模式菜单首项变「完成编辑」/ 保存按钮仅「保存」二字 / 无 Token 保存与删除拦截（弹设置）/ 回到主页侧栏重置
