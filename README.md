...
- **🏠 主页 · 四季画廊（v6.6 液态玻璃重设计）**：打开平台即见全屏主页——赛里木湖壁纸打底（`assets/home-bg.jpg`，**固定不可滑动**），顶部→中部清晰展示壁纸与标题「四季路线」，中部→底部白色渐变 + backdrop 磨砂，到底部约 25% 半透明呼吸感。路线按**春夏秋冬四个板块**从上到下排列（归属读 catalog 条目 `season` 字段，未登记默认归春；空季节显示「静待规划」虚线占位卡）。四季板块整体控制在视口垂直 40%–90% 区域（滚动层 top 40% / bottom 2%）；滚轮只滚动卡片层：卡片上移、板块顶缘越过视口 40% 线开始渐隐（JS 驱动 `applyHomeFade`：rAF 节流按位置计算 opacity/blur/translateY，到 ~12% 线完全消失），渐隐融入壁纸。每张卡显示路线名 + 「X 天 X 晚」；顶部三件套玻璃 pill：＋ 新建 / ⚙ 设置 / 🔄 刷新
