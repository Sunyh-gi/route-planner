...
- **🏠 主页 · 四季画廊（v6.6 液态玻璃重设计）**：打开平台即见全屏主页——赛里木湖壁纸打底（`assets/home-bg.jpg`，**固定不可滑动**），顶部→中部清晰展示壁纸与标题「四季路线」，壁纸整张无叠加直接展示（v6.7.5 起移除磨砂层与白渐变层）。路线按**春夏秋冬四个板块**从上到下排列（归属读 catalog 条目 `season` 字段，未登记默认归春；空季节显示「静待规划」虚线占位卡）。四季板块整体控制在视口垂直 40%–90% 区域（滚动层容器 top 12% / bottom 2%，padding-top 把内容起始垫到视口 40% → **渐隐全程发生在容器内部，无 overflow 裁切硬边**）；滚轮只滚动卡片层：卡片上移、板块顶缘越过视口 40% 线开始渐隐（JS 驱动 `applyHomeFade`：rAF 节流按位置计算 opacity/blur/translateY，到 ~12% 线完全消失，恰在容器裁切线前透明），渐隐融入壁纸；卡片清透玻璃 + 极轻液态玻璃折射（v6.8.2 终态）：SVG feDisplacementMap 边缘折射收敛到 bendPx 6px / bendAmp 3px 且无光学叠加（无 brightness/saturate/blur），底色 rgba(255,255,255,.05)/hover .11、细白边框 + 减弱的边缘受光细节（顶缘高光 .38 + 柔光带 + 下缘次高光）+ 极淡托影，无文字阴影；季节头与卡片文字为白色系（季节头 .96、卡名 .72、天数 .5），季节头无 emoji 图标，无文字阴影（纯液态玻璃质感）。每张卡显示路线名 + 「X 天 X 晚」；顶部三件套玻璃 pill：＋ 新建 / ⚙ 设置 / 🔄 刷新
- **路线界面（v6.4 视图/编辑双模）**：进入路线默认**视图模式**——只显示路线名 + ⋯ + 各天及地点（点击行飞往标记），搜索栏 / 保存按钮 / 内联编辑按钮（✎↑↓◀▶✕）/ 整行拖拽全部隐藏。点 ⋯ → **编辑路线** 切到**编辑模式**——搜索栏（液态玻璃 + 放大镜图标按钮）+ 保存按钮（仅「保存」二字 + 液态玻璃）+ 各天内联编辑按钮 + 拖拽换位/换天全部恢复。新建路线 / 副本 直接进入编辑模式。编辑模式下路线名旁出现灰色「编辑中」徽标
- **打开路线自动取景（v6.5.3）**：进入路线或复制副本后，地图自动 `fitBounds` 到适合范围：按视口 12%×15% 留白计算（Leaflet 内部自动 snap zoom 0.5 + 适配 bounds），再把视图中心相对路线中心**向西**偏移约 7.5% 视口宽（路线相对视口向右偏移）以补偿左侧 220px 半透明玻璃面板——路线在「扣除面板的可见区」内视觉居中，左缘不贴面板。bounds 跨度自适应（小范围自动放大、大跨度如伊犁 ~7° 自动降到 zoom 7.5）。编辑过程（加站 / 拖拽 / 换天）不打断用户视角；保存归位不重取景
- **搜索按钮（v6.4）**：放大镜从输入框内移到右侧独立按钮，液态玻璃半透明圆角按钮（与搜索框视觉一致）；输入框仅显示 placeholder「地名 / GPS 坐标」
- **保存按钮（v6.4）**：仅「保存」二字 + 液态玻璃半透明圆角（无 💾 / 无 "到 GitHub" 字样）
- **⋯ 头部菜单（v6.4：无图标三选项）**：✎ 编辑路线（进入编辑模式，搜索 / 保存 / 内联编辑按钮 / 拖拽全部出现；再次点击切回视图模式）/ 🗑️ 删除路线（删除需二次确认并消耗一次仓库写入；新路线 / 副本未入库不可删除）/ ⌂ 回到主页
- **浏览器前进/后退（v6.5.2 hash 路由）**：状态模型 `#/` 主页 · `#/n` 新建 · `#/r/<id>` 进入路线 · 设置弹层在任意页附加 `/s`（如 `#/r/<id>/s`）。所有界面切换都会把当前状态写入地址栏 hash 并加入浏览器历史——从主页进设置或路线后，按浏览器返回键即可退回主页；**F5 刷新停留在 `#/r/<id>`（或地址栏直达带 hash 的链接）也能恢复对应路线视图**。离开含未保存修改的页面会弹确认，取消则地址栏还原、留在原页
...
- **按天编排**：每个地点归属具体某一天，地点按天自动排序，段路径颜色取自全局 30 色调色板 `DAY_PALETTE`（色轮柔和蓝→柔紫→柔红）；**v6.5 起 30 色完整覆盖色轮左半圈** `h=214°(柔蓝) → h=360°(柔红)`，经紫/品红/玫红柔和过渡；**v6.4 起每天颜色按 routeId 种子按"间隔步长"洗牌后取**——同一路线内相邻天色相差 ≥ 17°（肉眼显著），不同路线起点不同
...
- **v6.9.2 定论（关键）**：清透 = 透明度，而非色彩对比度——暗调玻璃（深色图层感）已被否决回退；**全项目统一白玻璃配方**（与主页卡片同族）：L1 白.08+blur(3px)+白边框.32+受光，文字按底图场景选色（浅色底图深字 #333/#3a4658，壁纸上白字）；菜单白玻璃+深字、homeFab 白玻璃+深字；遮罩 .22 轻遮罩；L2 弹窗白.58+blur(20px) 高透过
...
- `_smoke.js`：三阶段冒烟（puppeteer-core + 系统 Edge，`file://` 直开场景）
  - **阶段 A**（18 项，无 Token 只读）：四季画廊两张卡（夏 yl 在前、秋 cx 在后）无徽章 / 目录已注册 / 主页三件套玻璃 pill（新建 / 设置 / 刷新）/ 点卡懒加载 cx 11 点进入视图模式（搜索 / 保存 / 编辑按钮 / 拖拽全隐藏、无 day-stepper）/ ⋯ 菜单三选项无图标 / 点「编辑路线」切编辑模式（66 个行内 ops + 11 个可拖拽行）/ 搜索预览 + 液态玻璃卡片 / 加站 12 点两天 dirty / 相邻天颜色按 routeId 种子洗牌显著不同 / 编辑模式菜单首项变「完成编辑」/ 保存按钮仅「保存」二字 / 无 Token 保存与删除拦截（弹设置）/ 回到主页侧栏重置
  - **阶段 B**（13 项，mock fetch api.github.com）：注入仓库配置 + Token / 保存 PUT 路线文件 + catalog / 侧栏 dirty 清 / enterRoute 回视图模式 / 改名带 sha 覆盖 / DELETE + 推目录回 cx,yl / 复制预填 dirty=true / 远端刷新拉取新路线 / 旧路线迁移入库 + 清空本地 / 主页 4 卡 + 截屏
  - **阶段 C**（10 项，hash 路由浏览器前进/后退 + 自动取景）：点卡 → `#/r/cx` 进路线 / 后退回主页 `#/` / 主页 ⚙ 设置 → `#/s` 叠加 / 后退关设置、前进重开 / 关闭按钮归位 / 手改 `#/n` 直达新建编辑模式 / 手改 `#/` 回主页 / F5 刷新 `#/r/cx` 恢复路线视图 / **自动取景（11 点全在视口 & zoom>4）**
  - 全部断言：**41 / 41 通过**，非网络控制台错误 0 条
